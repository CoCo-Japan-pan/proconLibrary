...
struct DST {
   ...
   public:
    DST(const std::vector<T> &vec, const OperatorType &op_) : size(vec.size()), op(op_) {
        int depth = 1;
        ...
        for(int a = 1; a < depth; a++) {
            ...
            for(int b = 0; b < size; b += (maxLen << 1)){
                ...
                for(int r = middle + 1; r < std::min(middle + maxLen, size); r++){
   ...
   private:
    ...
    std::vector<std::vector<T>> data; // data[i][j]で、[j, MSSB) または [MSSB, j]
